...
adb install -r "$APK" >"$TMP/install.log"
adb shell pm clear "$PACKAGE" >/dev/null || true
adb shell am start -n "$PACKAGE/$ACTIVITY" >"$TMP/am-start.log"
sleep 12
...
if adb shell run-as "$PACKAGE" pwd >/dev/null 2>&1; then
  adb shell run-as "$PACKAGE" sh -c 'find . -maxdepth 4 -type f | sort | sed -n "1,200p"' > "$TMP/private-files.txt" || true
